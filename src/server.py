...
# Network Hook Management Functions

@mcp.tool
async def create_request_hook(
    name: str,
    url_pattern: str,
    action: str,
    ...
    """
    Create a new request hook to intercept and modify outgoing network requests.
    
    Args:
        name (str): Human-readable hook name
        url_pattern (str): URL pattern to match (supports wildcards like *.example.com)
        action (str): Action to take - 'modify', 'block', 'redirect', 'fulfill', or 'log'
        modifications (Optional[Dict[str, Any]]): Modifications to apply (for modify action)
        instance_ids (Optional[List[str]]): Browser instances to apply hook to (all if None)
        resource_type (Optional[str]): Resource type filter (image, stylesheet, script, etc.)
        priority (int): Hook priority (lower = higher priority)
        
    Returns:
        Dict[str, Any]: Hook creation result with hook_id
    """
    return await network_hook_ai.create_request_hook(
        name=name,
...
@mcp.tool
async def create_response_hook(
    name: str,
    url_pattern: str,
    action: str,
    modifications: Optional[Dict[str, Any]] = None,
    ...
    """
    Create a new response hook to intercept and modify incoming network responses.
    
    Args:
        name (str): Human-readable hook name
        url_pattern (str): URL pattern to match
        action (str): Action to take - 'modify', 'block', 'fulfill', or 'log'
        ...
        resource_type (Optional[str]): Resource type filter
        priority (int): Hook priority
        
    Returns:
        Dict[str, Any]: Hook creation result with hook_id
    """
    return await network_hook_ai.create_response_hook(
        name=name,
        url_pattern=url_pattern,
        action=action,
        modifications=modifications,
...
@mcp.tool
async def create_redirect_hook(
    name: str,
    url_pattern: str,
    redirect_url: str,
    instance_ids: Optional[List[str]] = None,
    ...
    """
    Create a hook that redirects matching requests to a different URL.
    
    Args:
        name (str): Hook name
        url_pattern (str): Pattern to match
        redirect_url (str): URL to redirect to
        instance_ids (Optional[List[str]]): Browser instances
        priority (int): Hook priority
        
    Returns:
        Dict[str, Any]: Hook creation result
    """
    return await network_hook_ai.create_redirect_hook(
        name=name,
        url_pattern=url_pattern,
...
async def create_block_hook(
    ...
    """
    Create a hook that blocks matching requests.
    
    Args:
        name (str): Hook name
        url_pattern (str): Pattern to match and block
        instance_ids (Optional[List[str]]): Browser instances
        resource_type (Optional[str]): Resource type filter
        priority (int): Hook priority
    ...
    Returns:
        Dict[str, Any]: Hook creation result
    """
    return await network_hook_ai.create_block_hook(
        name=name,
        url_pattern=url_pattern,
        instance_ids=instance_ids,
        resource_type=resource_type,
...
@mcp.tool
async def create_custom_response_hook(
    name: str,
    url_pattern: str,
    ...
    """
    Create a hook that fulfills requests with custom responses.
    
    Args:
        name (str): Hook name
        url_pattern (str): URL pattern to match
        response_data (Dict[str, Any]): Response data (status, headers, body)
        instance_ids (Optional[List[str]]): Browser instances
        priority (int): Hook priority
    ...
    Returns:
        Dict[str, Any]: Hook creation result
    """
    return await network_hook_ai.create_custom_response_hook(
        name=name,
        url_pattern=url_pattern,
...
@mcp.tool
async def list_network_hooks(instance_id: Optional[str] = None) -> Dict[str, Any]:
    """
    List all network hooks with their details.
    
    Args:
        instance_id (Optional[str]): Filter by browser instance (all if None)
        
    Returns:
        Dict[str, Any]: List of hooks with details
    """
    return await network_hook_ai.list_hooks(instance_id)


@mcp.tool
async def get_network_hook_details(hook_id: str) -> Dict[str, Any]:
    """
    Get detailed information about a specific network hook.
    
    Args:
        hook_id (str): Hook identifier
        
    Returns:
        Dict[str, Any]: Detailed hook information
    """
    return await network_hook_ai.get_hook_details(hook_id)


@mcp.tool
async def remove_network_hook(hook_id: str) -> Dict[str, Any]:
    """
    Remove a network hook by ID.
    ...
    """
    return await network_hook_ai.remove_hook(hook_id)


@mcp.tool
async def update_network_hook_status(hook_id: str, status: str) -> Dict[str, Any]:
    """
    Update the status of a network hook.
    
    Args:
        hook_id (str): Hook identifier
        status (str): New status - 'active', 'inactive', or 'paused'
        
    Returns:
        Dict[str, Any]: Update result
    """
    return await network_hook_ai.update_hook_status(hook_id, status)


@mcp.tool
async def list_pending_requests(instance_id: Optional[str] = None) -> Dict[str, Any]:
    """
    List pending network requests in the interception queue.
    These are requests that have been paused and can be modified.
    
    Args:
        instance_id (Optional[str]): Filter by browser instance
        
    Returns:
        Dict[str, Any]: List of pending requests
    """
    return await network_hook_ai.list_pending_requests(instance_id)


@mcp.tool
async def get_pending_request_details(request_id: str) -> Dict[str, Any]:
    """
    Get detailed information about a pending request.
    ...
    """
    return await network_hook_ai.get_pending_request_details(request_id)


@mcp.tool
async def modify_pending_request(
    request_id: str,
    modifications: Dict[str, Any]
) -> Dict[str, Any]:
    """
    Modify a pending network request before it's executed.
    
    Args:
        request_id (str): Request identifier
        modifications (Dict[str, Any]): Modifications (url, method, headers, post_data, etc.)
        
    Returns:
        Dict[str, Any]: Modification result
    """
    return await network_hook_ai.modify_pending_request(request_id, modifications)


@mcp.tool
async def execute_pending_request(instance_id: str, request_id: str) -> Dict[str, Any]:
    """
    Execute a pending request with its current modifications.
    
    Args:
        instance_id (str): Browser instance ID
        request_id (str): Request identifier to execute
    ...
    return await network_hook_ai.execute_pending_request(tab, request_id)
